...
export default function SubscribeModal({ isOpen, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      {/* Backdrop */}
      <div 
        className="absolute inset-0 bg-[#0c0f0a]/50 backdrop-blur-sm transition-opacity duration-300"
        onClick={onClose}
      />

      {/* Modal Content */}
      <div className="relative w-full max-w-xl overflow-hidden rounded-3xl bg-background border border-foreground/5 p-8 sm:p-12 shadow-2xl transition-all duration-300 transform scale-100 animate-in fade-in zoom-in-95 duration-200">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute right-6 top-6 rounded-full p-2 text-foreground/50 hover:bg-foreground/5 hover:text-foreground transition-all duration-200"
        ...
        {status === "done" ? (
          ...
          <div className="flex flex-col items-center">
            ...
            {/* Main title */}
            <h3 className="mt-3 font-serif text-3xl sm:text-4xl font-bold tracking-tight text-foreground text-center leading-tight">
